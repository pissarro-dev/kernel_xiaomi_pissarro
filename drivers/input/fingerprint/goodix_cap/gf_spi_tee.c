				   ...
				   unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == MI_DISP_DPMS_EVENT && gf_dev) {
...
#endif //goodix_fb_state_chg_callback
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_INIT:
		...
		mi_disp_register_client(&gf_dev->notifier);
	...
	case GF_IOC_EXIT:
		...
		mi_disp_unregister_client(&gf_dev->notifier);
#endif
#endif
				 ...
				 size_t count)
{
	if (!strncmp(buf, "1", count)) {
		...
		//freq_hold(1);
		schedule_work(&fp_display_work);//notify display power on
...
static int gf_probe(struct spi_device *spi)
{
	...
	if (gf_dev->spi_buffer == NULL) {
		status = -ENOMEM;
		gf_debug(ERR_LOG, "malloc spi buffer fail");
		goto err_freqbuff;
	}
#ifdef CONFIG_MFD_MT6362
	buck = regulator_get(NULL, "mt6362-ldo1");
#else
	...
	if (1) {
		if (((rx_test[0] != 0x07) || (rx_test[3] != 0x25)) && ((rx_test[0] != 0x10) || (rx_test[3] != 0x25)) && ((rx_test[0] != 0x08) || (rx_test[3] != 0x25))) {
