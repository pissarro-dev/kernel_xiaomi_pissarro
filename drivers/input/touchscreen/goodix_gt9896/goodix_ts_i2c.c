...
static uint64_t write_start_time,write_end_time;
	...
	struct goodix_ts_board_data *board_data)
{
	...
	ts_info("get vdd-gpio[%d] from dt", r);
	...
	/* new touchfeature default parameter */
	r = of_property_read_u32_array(node, "goodix,touch-game-param-config1",
			board_data->gamemode_config1, 4);
	if (!r) {
		ts_info("read touch gamemode parameter config1:[%d, %d, %d, %d]",
		board_data->gamemode_config1[0], board_data->gamemode_config1[1],
		board_data->gamemode_config1[2], board_data->gamemode_config1[3]);
	} else {
		ts_err("Failed to get touch gamemode parameter config1");
	}

	r = of_property_read_u32_array(node, "goodix,touch-game-param-config2",
			board_data->gamemode_config2, 4);
	if (!r) {
		ts_info("read touch gamemode parameter config2:[%d, %d, %d, %d]",
		board_data->gamemode_config2[0], board_data->gamemode_config2[1],
		board_data->gamemode_config2[2], board_data->gamemode_config2[3]);
	} else {
		ts_err("Failed to get touch gamemode parameter config2");
	}

	r = of_property_read_u32_array(node, "goodix,touch-game-param-config3",
			board_data->gamemode_config3, 4);
	if (!r) {
		ts_info("read touch gamemode parameter config3:[%d, %d, %d, %d]",
		board_data->gamemode_config3[0], board_data->gamemode_config3[1],
		board_data->gamemode_config3[2], board_data->gamemode_config3[3]);
	} else {
		ts_err("Failed to get touch gamemode parameter config3");
	}
	...
	ts_debug("[DT]x:%d, y:%d, w:%d, p:%d", board_data->panel_max_x,
		board_data->panel_max_y, board_data->panel_max_w, board_data->panel_max_p);
	return 0;
}
#endif
	...
	unsigned char *data, unsigned int len)
{
	...
	if (likely(len < limit_size)) {
		/* code optimize, use stack memory */
		msgs[1].buf = kzalloc(I2C_MAX_TRANSFER_SIZE < len
				? I2C_MAX_TRANSFER_SIZE : len, GFP_DMA);
		if (msgs[1].buf == NULL)
			return -ENOMEM;
	} else {
		msgs[1].buf = kzalloc(I2C_MAX_TRANSFER_SIZE < len
		...
		struct goodix_ts_config *config)
{
	...
	mutex_unlock(&dev->charger_mutex);
	return r;
}
	...
	struct goodix_touch_data *touch_data, unsigned char *buf, int touch_num)
{
	...
	core_data->fod_area = *(coor_data + touch_num * BYTES_PER_COORD + 2);
	/*ts_info("fod_area = %d", core_data->fod_area);*/
	for (i = 0; i < touch_num; i++) {
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	static u8 buffer[IRQ_HEAD_LEN_YS +
			 BYTES_PER_COORD * GOODIX_MAX_TOUCH + 3];
		...
		struct goodix_ts_event *ts_event)
{
	...
	if (dev->ic_type == IC_TYPE_YELLOWSTONE) {
		pre_read_len = IRQ_HEAD_LEN_YS + BYTES_PER_COORD + 3;
	} else {
	...
	if (likely((event_sta & GOODIX_TOUCH_EVENT) == GOODIX_TOUCH_EVENT)) {
		/* handle touch event */
		if (dev->ic_type == IC_TYPE_YELLOWSTONE) {
			goodix_touch_handler_ys(dev, ts_event, pre_buf, pre_read_len);
		} else {
			goodix_touch_handler_nor(dev, ts_event, pre_buf, pre_read_len);
		}
	...
	const struct i2c_device_id *dev_id)
{
	...
	mutex_init(&ts_device->charger_mutex);
