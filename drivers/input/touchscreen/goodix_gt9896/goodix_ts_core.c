		...
		struct goodix_touch_data *touch_data)
{
	...
	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		...
		if((touch_data->coords[i].x - pre_coords[i].x ||
				...
				pre_coords[i].status == 0) {
			ts_info("finger report move:%d", i);
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	//pm_qos_update_request(&core_data->tp_qos_request, 500);
#ifdef CONFIG_PM
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (strlen(ts_bdata->avdd_name)) {
		...
		r = regulator_set_voltage(core_data->avdd, 3300000, 3300000);
		if (r) {
...
static void goodix_power_supply_work(struct work_struct *work)
{
	...
	if (wakeup_complete == false) {
		ts_err("tp is suspend,do not set register\n");
		return;
	}
	...
	ts_info("%s:cur_chgr.intval = %d\n", __func__, cur_chgr.intval);
...
static struct notifier_block goodix_noti_block = {
	.notifier_call = goodix_ts_fb_notifier_callback,
};
#endif
...
#ifdef CONFIG_DRM
	core_data->fb_notifier = goodix_noti_block;
	if (mi_disp_register_client(&core_data->fb_notifier)) {
...
static int goodix_palm_sensor_write(int val) {
	ts_info("palm sensor value : %d", val);
...
out:
	if (r) {
		...
		power_supply_unreg_notifier(&core_data->power_supply_notif);
		goodix_ts_power_off(core_data);
	...
	if (r == 0){
		ts_info("goodix_ts_probe OUT,---success---\n");
		tp_load_ok = true;
		wakeup_complete = true;
	} else {
		ts_info("goodix_ts_probe OUT,---fail--- r:%d", r);
		tp_load_ok = false;
	}
