...
enum product_name product_name = PISSARRO;
...
static void max77932_parse_cmdline(void)
{
	...
	max_info("pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
...
static int max77932_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (product_name == PISSARROPRO) {
		...
		max_info("PISSARRO no need to probe MAX77932\n");
		return -ENODEV;
	}
	...
	wakeup_source_init(&max->irq_handle_wakelock, "max77932_irq_handle_wakelock");
