...
static int get_quick_charge_type(struct charger_manager *info)
{
	...
	if (!info || !info->usb_psy || !info->xmusb350_psy || !info->bms_psy)
	...
	if (psy_type == POWER_SUPPLY_TYPE_USB_PD && info->pd_verifed) {
		if (info->apdo_max >= WIRE_SUPER_POWER_MAX) /* if pd power max larger than 50W report super charge */
...
static int get_adapter_power_max(struct charger_manager *info)
{
	int apdo_max = 0;
	...
	if (mt_charger_plugin()) {
		apdo_max = info->apdo_max;
		pr_info("apdo_max:%d\n", apdo_max);

		if (apdo_max == 100 || apdo_max == 105)
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_POWER_MAX:
		val->intval = get_adapter_power_max(info);
		if (info) {
			if (product_name == PISSARROPRO)
				val->intval = min(val->intval, 120);
			else
				val->intval = min(val->intval, 67);
		} else
			val->intval = 0;
		break;
